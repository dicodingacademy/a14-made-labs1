            ...
            R.id.btn_choose -> {
                ...
                if (checkedRadioButtonId != -1) {
                    ...
                    when (checkedRadioButtonId) {
                        R.id.rb_saf -> coach = rbSaf.text.toString().trim { it <= ' ' }

                        R.id.rb_mou -> coach = rbMou.text.toString().trim { it <= ' ' }

                        R.id.rb_lvg -> coach = rbLvg.text.toString().trim { it <= ' ' }

                        R.id.rb_moyes -> coach = rbMoyes.text.toString().trim { it <= ' ' }
                    }
